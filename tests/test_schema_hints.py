...
own entry point (the `ledger` fixture, in `conftest.py`). The adversarial suite's
throwaway ledger cannot answer for this file: it hand-rolls relations that only
have to be *shaped* like the real ones (its `payments` has three columns; the
real one has six), so a column list checked against it would prove nothing about
